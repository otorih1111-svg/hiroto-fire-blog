...
BLOCKED_THEME_CLUSTERS = [
    ...
    "AIに愚痴を話した",
    "AIに今日何すればいいか聞いた",
    # 2026-06-07追加：新NISA×家計整理クラスター（既に3本以上あり、これ以上は厳禁）
    "新NISAを始める前に家計を見直した・整理した話",
    "新NISA 家計 見える化 固定費 投資額の決め方",
    "新NISAを続けるために家計を整えた話",
    # 2026-06-07追加：楽天経済圏×固定費クラスター（既に3本以上あり、これ以上は厳禁）
...
- タイトルは32文字以内を最優先し、長くなる場合は削る
- descriptionは70〜120文字で、悩みとベネフィットが両方入るようにする
- 【🚫タイトル型の重複禁止・2026-06-07追加】「〇〇した話【40代シングル父の実録】」「〇〇した話【シングルファーザー実録】」のような「【〇〇実録】」で終わるタイトルは、サイト内に既に複数本（fukugyo3・aifukugyo・nisa・rakuten・katte-yokatta-mac等）あり、これ以上は使わないこと。
  - タイトルは「実録」という単語に頼らず、「ベネフィット」「悩み→変化」「数字」「問いかけ」など別の型で表現すること
  - 例：「ふるさと納税を初めてやってみた話【〇〇実録】」ではなく「シングルファーザーが初めてふるさと納税やった話」のように、体験は伝えつつ末尾の【】テンプレを使わない
  - 直近の公開済み記事タイトルと末尾の型（【】の中身・文末表現）が同じにならないか必ず確認すること
...
YMYLジャンルの記事では：
1. **タイトルに体験を示す言葉を入れる**：「やってみた」「気づいた」「正直に書く」「初めて」など。ただし「実録」は既に複数記事で使用済みのため、新規記事のタイトルでは使わないこと
...
- タイトルが32文字以内か
- タイトルが「【〇〇実録】」型など、既存記事と同じ末尾テンプレになっていないか
- descriptionが70〜120文字か
- 内部リンクが2本以上あるか
- CTAが「考えてみて」で終わっていないか

## 出力形式
frontmatterなしで、H1タイトルから始めてください。
記事の長さ：5000文字前後（最低4000文字・最大6000文字）"""
    ...
    blocked_str += "- 「新NISAを始める前/続けるために家計を整理・見直しした話」系全般（既に3本以上あり追加禁止）\n"
    blocked_str += "- 「楽天経済圏で固定費がじわじわ増えた/2年使って気づいた」系全般（既に3本以上あり追加禁止）\n"
    blocked_str += "上記に近いテーマが浮かんだ場合は、全く別の切り口・テーマを選ぶこと。\n"
    blocked_str += (
        "\n## 🔑 KW確認ゲート（必須・例外なし）\n"
        "- このテーマで書く前に「想定メインKW（2〜4語の組み合わせ）」を1つ自分で決めること\n"
        "- 決めたKWが、上の『公開済み記事』リストの内容と検索意図が重なる場合は、そのテーマでは書かないこと\n"
        "- 検索意図が重ならない、新しい角度のKWであることを確認してから本文を書き始めること\n"
        "- 1記事1KWを徹底し、似たKW（例：『新NISA 家計見直し』と『新NISA 始める前 家計』）は同じ記事として扱うこと\n"
    )
