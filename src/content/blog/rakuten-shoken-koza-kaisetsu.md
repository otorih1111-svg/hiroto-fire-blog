...
2. 入金方法を設定する（楽天銀行との連携、または楽天カードでのクレカ積立）
3. 積み立てる投資信託を選ぶ
4. 毎月の金額と積立日を設定する

### 入金方法は「楽天カードのクレカ積立」にした

入金方法はいくつか選べますが、ぼくは楽天カードでのクレカ積立を選びました。

理由は単純で、毎月の引き落としを自動化できて、ポイントもつくからです。

ただし、クレカ積立には毎月の上限額があります。

ぼくは最初、「とりあえず上限いっぱいまで設定しよう」と思ったのですが、家計の負担になりそうだったので、無理のない金額に下げました。

「ポイントがつくから多めに設定したい」という気持ちと、「家計を圧迫しない金額にする」という冷静さは、わりと喧嘩します。

最終的には、後者を優先しました。

![口座開設後にやったこと（4ステップ）](/images/articles/rakuten-shoken-koza-kaisetsu/after-setup.svg)

### 銘柄選びと積立設定の3ステップ

この3つだけ終わらせれば、あとは設定が自動で動いてくれます。

1. 検索窓で「オルカン」または「S&P500」と入力する
2. 表示された投資信託の「積立設定」ボタンを押す
3. 毎月の金額・積立日・引き落とし方法を選んで確定する

ぼくが実際にどの投資信託を選んで、どんな積立設定にしたかは、こちらの記事に詳しく書いています。
